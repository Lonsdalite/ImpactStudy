...
export function BillingSettingsForm({
  ...
  function submit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const res = await updateBilling(studentId, yearLevel, cycle, anchor);
      if (res.ok) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-3 text-xs text-brand-ink/45">
        Year level picks the price-list rate for each enrollment. Cycles are
        counted from the start date; fees are billed in arrears and due at the
        end of each cycle.
      </p>
    </form>
  );
